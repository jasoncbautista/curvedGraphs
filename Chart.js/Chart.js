...
window.Chart = function(context){
	...
	this.Line = function(data,options){
		...
		chart.Line.defaults = {
			...
			scaleFontSize : 12,
			scaleFontStyle : "normal",
			scaleFontColor : "#666", // Color of labels
	...
	var PolarArea = function(data,config,ctx){
		...
		function drawAllSegments(animationDecimal){
			...
			for (var i=0; i<data.length; i++){
				...
				ctx.fillStyle = red; //data[i].color;
				ctx.fill();
		...
		function drawLines(animPc){
			...
			for (var i=0; i<data.datasets.length; i++){
				...
				if (config.datasetFill){
					...
					ctx.closePath();
					ctx.fillStyle = data.datasets[i].fillColor; // Color of graph
					ctx.fill();
				}
				else{
					ctx.closePath();
                ...
                if(config.pointDot){
                    for (var k=0;  k<data.datasets[i].data.length; k++){
                        ...
                        if ( data.datasets[i].selectedPointIndex === k) {
                            ...
                            ctx.fillStyle =   data.datasets[i].pointColor;
                            ctx.strokeStyle =  data.datasets[i].pointStrokeColor; ctx.lineWidth = config.pointDotStrokeWidth;
                        }
